...
function wpcfe_print_shipment_ajax_callback(){
    ...
    try {
        // Validar entrada
        $shipment_id = absint( $_POST['shipmentID'] ?? 0 );
        ...
        if ( ! is_wpcfe_shipment( $shipment_id ) ) {
            throw new Exception('Shipment not found');
        }
        
        // Variables
        $wpcfe_pdf_dpi  = apply_filters( 'wpcfe_pdf_dpi', 160 );
        $print_paper    = wpcfe_print_paper()[$print_type] ?? null;
        
        if ( ! $print_paper ) {
            throw new Exception('Invalid print type configuration');
        }
...
function wpcfe_print_shipment_template_path( $shipment_id, $waybill_title, $print_type ){
    try {
      ...
      ?>
        ...
        ob_end_clean();
        error_log('WPCFE Template Error: ' . $e->getMessage());
        throw $e;
    }
}