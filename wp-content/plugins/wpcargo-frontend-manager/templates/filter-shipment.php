...
$has_filters    = ! empty( array_filter( array(
	$s_status,
	$s_shipper,
	$s_receiver,
	isset( $_GET['date_start'] ) ? sanitize_text_field( $_GET['date_start'] ) : '',
	isset( $_GET['date_end'] ) ? sanitize_text_field( $_GET['date_end'] ) : '',
	isset( $_GET['shipment_branch'] ) ? sanitize_text_field( $_GET['shipment_branch'] ) : '',
	isset( $_GET['tipo_envio'] ) ? sanitize_text_field( $_GET['tipo_envio'] ) : '',
	isset( $_GET['lugar_origen'] ) ? sanitize_text_field( $_GET['lugar_origen'] ) : '',
	isset( $_GET['lugar_destino'] ) ? sanitize_text_field( $_GET['lugar_destino'] ) : '',
	isset( $_GET['driver_recojo'] ) ? sanitize_text_field( $_GET['driver_recojo'] ) : '',
	isset( $_GET['driver_entrega'] ) ? sanitize_text_field( $_GET['driver_entrega'] ) : ''
) ) );

?>
<div class="col-lg-9 col-md-8 mt-0 wpcfe-desktop-filter-col">
	<div id="wpcfe-filters-home">
...
<div class="col-lg-3 col-md-4 mt-0 p-0">
	<form id="wpcfe-search" class="float-md-none float-lg-right" action="<?php echo esc_url( $page_url ); ?>" method="get">
		<div class="form-sm wpcfe-mobile-search-bar">
			...
			<div class="wpcfe-mobile-search-actions">
				<button type="submit" class="btn btn-primary btn-sm mx-md-0 ml-2 wpcfe-mobile-search-submit"><?php esc_html_e('Search', 'wpcargo-frontend-manager' ); ?></button>
				<button type="button" class="btn btn-outline-primary btn-sm wpcfe-mobile-filter-trigger d-md-none d-lg-none" data-toggle="modal" data-target="#wpcfeMobileFiltersModal"><?php esc_html_e('Filters', 'wpcargo-frontend-manager' ); ?></button>
			</div>
		</div>
	</form>
</div>
<div class="modal fade" id="wpcfeMobileFiltersModal" tabindex="-1" role="dialog" aria-hidden="true">
	<div class="modal-dialog modal-dialog-scrollable" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title"><?php esc_html_e('Filters', 'wpcargo-frontend-manager'); ?></h5>
