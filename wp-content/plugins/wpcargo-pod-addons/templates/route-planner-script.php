<script>
    ...
    async function updateDeliveryStatus(shipmentId, newStatus) {
        ...
        const normalizedStatus = newStatus.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toUpperCase().trim();
            ...
            jQuery.ajax({
                ...
                success: function(response) {
                    try {
                        ...
                        // Bloquear el campo de estado a "Entregado" en el formulario cargado
                        const $form = jQuery('#wpc_pod_signature-modal #wpc_pod_signature-form');
                        const $status = $form.find('#status');
                        $status.val('Entregado').trigger('change');
                        $status.prop('disabled', true).addClass('disabled');
                        let $hiddenStatus = $form.find('input[name="status"][data-autogenerated="1"]');
                        if (!$hiddenStatus.length) {
                            $hiddenStatus = jQuery('<input>', {
                                type: 'hidden', name: 'status',
                                value: 'Entregado', 'data-autogenerated': '1'
                            });
                        ...
                        // El envío del formulario lo maneja pod-scripts.js via acción pod_signed
